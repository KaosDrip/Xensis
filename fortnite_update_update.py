...
async def admin_user_options(setting: str, user: fortnitepy.ClientUser, ctx: fortnitepy.ext.commands.Context) -> None:
    match setting.lower():
        case 'add':
            ...
            try:
                ...
                await ctx.send("Took long to responed. Re-Enter command and try again!")
                return
...
async def admin_user_setting_options(setting: str, user: fortnitepy.ClientUser, ctx: fortnitepy.ext.commands.Context) -> None:
    match setting.lower():
        case 'add':
            if ctx.message.author.id not in info['FullAccess']:
...
async def admin(ctx, setting = None, *, user = None):
    if not setting and not user:
        await ctx.send(f"Missing one or more arguments. Try: {prefix}admin (add, remove, list) (user)")
        return
    
    if setting and not user:
        user = await client.fetch_profile(ctx.message.author.id)
        await admin_user_options(setting, user, ctx)
        return
    
    if setting and user:
        user = await client.fetch_profile(user)
        await admin_user_setting_options(setting, user, ctx)
